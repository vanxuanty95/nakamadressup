...
class Item extends CI_Model
{
    ...
    */
    public function exists($item_id, $ignore_deleted = FALSE, $deleted = FALSE)
    {
        ...
        log_message("debug", "ctype_digit($item_id)" . $item_id . "-" . ctype_digit($item_id));
    ...
    */
    public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
    {
        ...
        if ($count_only == TRUE) {
            ...
            $this->db->select('MAX(items.consignmenter_id) AS consignmenter_id');
            ...
            $this->db->select('MAX(consignmenters.consignmenter_name) AS consignmenter_name');
        ...
        $this->db->join('consignmenters AS consignmenters', 'consignmenters.person_id = items.consignmenter_id', 'left');
    ...
    */
    public function get_info($item_id)
    {
        ...
        $this->db->select('consignmenters.consignmenter_name');
        $this->db->from('items');
        $this->db->join('consignmenters', 'consignmenters.person_id = items.consignmenter_id', 'left');
    ...
    */
    public function get_multiple_info($item_ids, $location_id)
    {
        $format = $this->db->escape(dateformat_mysql());
        $this->db->select('items.*');
        $this->db->select('consignmenter_name');
        $this->db->select('GROUP_CONCAT(DISTINCT CONCAT_WS(\'_\', definition_id, attribute_value) ORDER BY definition_id SEPARATOR \'|\') AS attribute_values');
    ...
    */
    ...
    public function get_search_suggestions($search, $filters = array('is_deleted' => FALSE, 'search_custom' => FALSE), $unique = FALSE, $limit = 25)
    {
        ...
        if (!$unique) {
            ...
            foreach ($this->db->get()->result() as $row) {
                $suggestions[] = array('label' => $row->consignmenter_name);
            }
     ...
     *
     */
    public function change_cost_price($item_id, $items_received, $new_price, $old_price = NULL, $total)
    {
        if ($old_price === NULL) {
