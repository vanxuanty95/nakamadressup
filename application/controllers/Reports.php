...
class Reports extends Secure_Controller
{
    ...
    public function date_input_consignmenters()
    {
        $data = array();
        $stock_locations = $data = $this->xss_clean($this->Stock_location->get_allowed_locations('receivings'));
        $stock_locations['all'] = $this->lang->line('reports_all');
        $data['stock_locations'] = array_reverse($stock_locations, TRUE);
        $data['mode'] = 'consignmenters';
    ...
    public function graphical_summary_consignmenters($start_date, $end_date, $sale_type, $location_id = 'all')
    {
        ...
        foreach ($report_data as $row) {
            ...
            $labels[] = $row['consignmenter'];
            $series[] = array('meta' => $row['consignmenter'] . ' ' . round($row['total'] / $summary['total'] * 100, 2) . '%', 'value' => $row['total']);
        }
    ...
    public function detailed_consignmenters($start_date, $end_date)
    {

        $this->load->model('reports/Detailed_consignmenters');
        $model = $this->Detailed_consignmenters;
        ...
        $data = array(
            'title' => $this->lang->line('reports_detailed_receivings_report'),
            'subtitle' => $this->lang->line('reports_detailed_receivings_report'),
            'headers' => $headers,
            'editable' => 'receivings',
            'summary_data' => $summary_data,
            'details_data' => $details_data,
