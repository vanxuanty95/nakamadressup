...
class Receiving_lib
{
    ...
    public function add_item($item_id, $quantity = 1, $item_location = NULL, $discount = 0, $fee = 0, $discount_type = 0, $price = NULL, $description = NULL, $serialnumber = NULL, $receiving_quantity = NULL, $receiving_id = NULL, $include_deleted = FALSE)
    {
        ...
        log_message("debug", '172 before'. $item_id. $include_deleted);
        if (!$this->CI->Item->exists($item_id, $include_deleted)) {
            log_message("debug", '172 into'. $item_id. $include_deleted);
        ...
        if ($itemalreadyinsale) {
            ...
            $items[$updatekey]['total'] = $this->get_item_total($items[$updatekey]['quantity'], $price, $discount, $fee, $discount_type, $items[$updatekey]['receiving_quantity']);
        } else {
    ...
    public function edit_item($line, $description, $serialnumber, $quantity, $discount, $fee, $discount_type, $price, $receiving_quantity)
    {
        $items = $this->get_cart();
        if (isset($items[$line])) {
            $line = &$items[$line];
            ...
            $line['price'] = $price;
            $line['total'] = $this->get_item_total($quantity, $price, $discount, $fee, $discount_type, $receiving_quantity);
            $this->set_cart($items);
        }
    ...
    public function add_item_kit($external_item_kit_id, $item_location, $discount, $fee, $discount_type)
    {
        //KIT #
